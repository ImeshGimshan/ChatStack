import { BadRequestException, Body, Controller, Get, InternalServerErrorException, Param, ParseIntPipe, Post, Req, UseGuards } from '@nestjs/common';
...
export class ChatController {
    ...
    async markMessageRead(
        ...
        try {
            if (dto.roomType === 'channel') {
                return await this.chatService.markChannelMessageRead(BigInt(dto.messageId), BigInt(userId));
            } else if (dto.roomType === 'conversation') {
                return await this.chatService.markConversationMessageRead(BigInt(dto.messageId), BigInt(userId));
            } else {
    ...
    async getChannelUnreadCount(
        ...
        try {
            const unreadCount = await this.chatService.getChannelUnreadCount(BigInt(id), BigInt(userId));
    ...
    async getConversationUnreadCount(
        ...
        try {
            const unreadCount = await this.chatService.getConversationUnreadCount(BigInt(id), BigInt(userId));
    ...
    async getConversationMessageReadReceipts(
        ...
        try {
            return this.chatService.getConversationMessageReadReceipts(BigInt(messageId));
        } catch (error: any) {
            console.error('Error fetching read receipts:', error);
            throw new InternalServerErrorException('Failed to fetch read receipts');
        }
    ...
    @Get('unread-messages')
    @ApiOperation({ summary: 'Get all unread channel messages for the authenticated user'})
    async getUnreadMessages(@Req() req: any) {
        ...
        try {
            const unreadChannelMessages = await this.chatService.getUserUnreadChannelMessages(BigInt(userId));
            const unreadConversationMessages = await this.chatService.getUserUnreadConversationMessages(BigInt(userId));
